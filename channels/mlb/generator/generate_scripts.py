...
日本のMLBファン向けに、長尺1〜2本とShorts 2〜3本の台本を作成してください。日本人選手を優先しつつ、事実パケットに海外選手の大記録や注目試合が含まれる場合は採用して構いません。
...
- 各セクションとShortsにimage_queryを必ず付ける。人物が主題なら入力にあるplayer_name_enを積極的に使い、Wikimedia Commonsのライセンス確認済み写真を探す。該当写真がなければbaseball stadium、baseball glove、pitching moundなどへフォールバックする。球団ロゴ、試合中継は指定しない。
- visual.labelsは入力にある数値や短い日本語だけにする。
- 選手や球団への中傷、根拠のない将来予測、賭博の推奨をしない。
- VOICEVOX向けに英字略称を読み上げない。MLBは「メジャーリーグ」と言い換える。
- 人名は入力のplayer_nameとplayer_name_enに完全一致させ、漢字や読みを推測しない。
