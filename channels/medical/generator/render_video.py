...
from generator.config import FFMPEG_BIN, FFMPEG_TIMEOUT

from generator.slides import (
    make_abstract_panel_bg,
    make_background,
    make_section_slide,
    make_short_slide,
    section_panel_box,
...
LONG_SUBTITLE_STYLE = (
    ...
    "Alignment=2,MarginL=42,MarginR=42,MarginV=46,Spacing=0.2"
)

SHORT_SUBTITLE_STYLE = (
    "FontName=Noto Sans CJK JP,FontSize=22,Bold=1,PrimaryColour=&H00FFFFFF,"
    "OutlineColour=&H00101010,BorderStyle=1,Outline=1.0,Shadow=0,"
    "Alignment=2,MarginL=96,MarginR=96,MarginV=150,Spacing=0.1,WrapStyle=2"
)
...
def _run_ffmpeg(cmd: list, timeout: int = FFMPEG_TIMEOUT) -> None:
    try:
...
def main(script_path=None):
    ...
    make_background(long_w, long_h, long_bg)
    ...
    for i, section in enumerate(data["long_sections"]):
        ...
        make_section_slide(
            long_w, long_h,
            section.get("source", "medical"),
            data["title"], bullets, i, fg_path,
        ...
        if not valid_video(clip_path):
            compose_full_frame_motion(long_bg, motion_path, fg_path, clip_path, long_box, long_w, long_h, duration)
        else:
            print(f"[checkpoint] reusing valid {clip_path}")
        clip_paths.append(clip_path)

    long_output = video_dir / "long.mp4"
    if valid_video(long_output):
        print(f"[checkpoint] reusing valid {long_output}")
    else:
        concat_clips_with_audio_and_subtitles(
            clip_paths, audio_dir / "long.wav", audio_dir / "long.srt", long_output, LONG_SUBTITLE_STYLE,
        )
        print(f"[info] wrote {long_output}")

    short_w, short_h = SHORT_SIZE
    for i, short in enumerate(data["short_scripts"], start=1):
        fg_path = video_dir / f"short_{i}_fg.png"
        make_short_slide(short_w, short_h, short.get("source", "medical"), short["hook"], fg_path)
        ...
        short_output = video_dir / f"short_{i}.mp4"
        if valid_video(short_output):
            print(f"[checkpoint] reusing valid {short_output}")
        else:
            concat_clips_with_audio_and_subtitles(
                [clip_path], short_wav, audio_dir / f"short_{i}.srt", short_output, SHORT_SUBTITLE_STYLE,
            )
            print(f"[info] wrote {short_output}")
