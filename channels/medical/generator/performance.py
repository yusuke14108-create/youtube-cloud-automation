...
def _sources_summary(source_items: list) -> str:
    return "+".join(sorted({s.get("source", "?") for s in source_items})) or "?"


def _describe_long(run: dict) -> dict:
    script = run["script"]
    sections = script.get("long_sections", [])
    source_items = script.get("source_items", [])
    return {
        "video_id": run["uploads"]["long"],
        "kind": "long",
        "title": script.get("title", ""),
        "topic_count": len(source_items),
        "section_count": len(sections),
        "sources": _sources_summary(source_items),
    }
...
def build_digest() -> str:
    ...
    for r in top:
        lines.append(
            ...
            f"トピック数={r['topic_count']}, 出典={r['sources']})"
        )
    for r in bottom:
        lines.append(
