    ...
    "properties": {
        ...
        "long_sections": {
            ...
            "items": {
                ...
                "properties": {
                    "bullet": {"type": "string"},
                    "narration": {"type": "string", "minLength": 180},
...
GENERATE_PROMPT_TEMPLATE = """次のニュース候補を起点に、日本人選手を軸としつつNBA全体も扱う解説動画の台本一式を作成してください。
...
- image_queryはWikimedia Commonsでライセンス確認するための英語検索語にする。人物が主題なら選手の英語名を積極的に使い、該当写真がなければbasketball hoop、basketball court、arena exteriorのみにフォールバックする。soccer、football、baseballなど他競技の語や画像は使わない。ロゴや試合中継画面は指定しない

作成するもの:
- title: 長尺動画のタイトル。冒頭に主役の選手名を置き、何が起きたかと見る理由が一目で分かる形にする（誇張しすぎず32〜45字程度）
- thumbnail_text: サムネイルに載せる短い文字（10字前後。煽りすぎない）
- thumbnail_image_query: ライセンス確認済み静止画を探す英語検索語（2〜4語）。人物が主題なら選手の英語名を第一候補にする
- summary: このニュースと分析の要約（200字程度）
- long_sections: 長尺動画（3〜5分）を4〜5個のセクションに分けた配列。各セクションは以下を持つ:
  ...
  - image_query: ライセンス確認済み静止画を探す英語検索語（2〜4語）。人物が主題なら選手の英語名を使う。全セクションに必須。
  ...
  全ナレーションは合計1100〜1600字を目安にする。最初の10秒で選手名と結論を提示し、続いて数字・役割・背景、最後に今後の注目点をまとめる。前置きや同じ説明の繰り返しは避ける。
- short_scripts: 20〜35秒のショート動画を{short_count}本（各90〜170字）。互いに違う切り口にする。hookは必ず主役の選手名から始め、「何が意外なのか」「何が重要なのか」が一読で分かる疑問形または結論形にする。「話題です」「ポイントとは」「転換期です」のような具体性のない表現は禁止。台本の最初の一文で選手名と結論を伝え、背景、根拠、視聴者が覚えるべき結論まで各Shorts内で完結させる。長尺を見ないと意味が通らない予告編にしない。長尺への誘導は必須ではなく、入れる場合も結論後の一言だけにする。
