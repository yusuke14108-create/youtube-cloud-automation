...
- short_scripts: 20〜35秒のショート動画を{short_count}本（各90〜170字）。互いに違う切り口にする。hookは必ず主役の選手名から始め、「何が意外なのか」「何が重要なのか」が一読で分かる疑問形または結論形にする。「話題です」「ポイントとは」「転換期です」のような具体性のない表現は禁止。台本の最初の一文でも選手名と結論を伝え、最後は「詳しくは関連動画へ」など短い長尺誘導で終える。
